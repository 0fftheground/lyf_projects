SELECT e.DeliveryTime, e.PickEmpName, ShelfAreaName, sumPickQty,sku_count, sum_resttime, worktime, (worktime-sum_resttime) as actual_worktime from 
...
INNER JOIN 
(select DeliveryTime, PickEmpName, ShelfAreaName, sum(PickQty) as sumPickQty, TIMESTAMPDIFF(SECOND , MIN(StartPickingTime),MAX(EndPickingTime)) AS worktime
...
group BY DeliveryTime,PickEmpName,ShelfAreaName) d 
on e.DeliveryTime = d.DeliveryTime and e.PickEmpName=d.PickEmpName 


INNER JOIN
(Select f.DeliveryTime,f.PickEmpName, COUNT(DISTINCT f.SKU) as sku_count from
(select DeliveryTime, PickEmpName,SKU
from xsyx_f6_picking.t_product_pick where OpAreaID = 101 and BatPickStatus = 3 and WID = 10139 and (DeliveryTime BETWEEN '2020-11-01 00:00:00' and '2021-04-14 23:59:59') 
and (StartPickingTime BETWEEN date_sub(DeliveryTime,interval 1 hour) and date_add(DeliveryTime,interval 11 hour))) f
Group By f.DeliveryTime,f.PickEmpName) g
on e.DeliveryTime = g.DeliveryTime and e.PickEmpName=g.PickEmpName 
